...
- parser block는 구조 계약으로 취급하고, sample corpus는 harness fixture 기준으로 관리한다.
- UI는 내부 모듈을 직접 읽지 않고 HTTP API만 호출한다.

## Change Implications
- parser 규약을 바꾸면 parser block 선언, harness sample corpus, parser 테스트, sample verification이 같이 바뀌어야 한다.
