...
## 목표
parser의 editor별 if-chain을 `ContainerBlock` / `LeafBlock` 기반 block dispatch 구조로 바꾸고, unsupported representative case 로직은 raw HTML 보존 + warning 노출 정책으로 단순화한다.

## 변경 범위

| 영역 | 현재 | 변경 후 | 기준 |
| --- | --- | --- | --- |
| Block 구조 | editor 내부 if-chain이 직접 AST를 만든다. | `parser/blocks/{common,naver-se2,naver-se3,naver-se4}` block module과 `supportedBlocks` dispatch로 분리한다. | [base-editor.ts](/Users/mj/projects/farewell-naver-blog/src/modules/parser/editors/base-editor.ts:15) |
| SE2 순회 | DOM node 재귀와 block 변환이 한 함수에 섞여 있다. | wrapper는 `ContainerBlock`, 실제 출력/skip/fallback은 `LeafBlock`이 처리한다. | [naver-blog-se2-editor.ts](/Users/mj/projects/farewell-naver-blog/src/modules/parser/editors/naver-blog-se2-editor.ts:332) |
| SE3/SE4 순회 | component 단위 if-chain으로 처리한다. | component 단위 `LeafBlock` 우선으로 옮기고 순서는 기존 동작과 같게 유지한다. | [naver-blog-se4-editor.ts](/Users/mj/projects/farewell-naver-blog/src/modules/parser/editors/naver-blog-se4-editor.ts:715) |
...
| 항목 | 내용 |
| --- | --- |
| Parser contract | 내부 `ParserBlock`, `ContainerBlock`, `LeafBlock`, `ParserBlockResult` 타입 추가 |
...
| 순회 타입 | `ContainerBlock`은 출력하지 않고 자식 node를 반환한다. |
| 소비 타입 | `LeafBlock`은 현재 node/component를 소비하고 `handled`, `skip`, `fallback` 결과를 반환한다. |
...
| 위험 | 방어 기준 |
| --- | --- |
| SE2 wrapper를 leaf로 소비해 내부 heading/image를 놓침 | `ContainerBlock` 우선순위와 기존 SE2 unit test 유지 |
