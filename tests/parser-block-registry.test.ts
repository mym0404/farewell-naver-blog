...
describe("parser block catalog", () => {
  it("keeps Naver editor instances and derives selectable output options by editor and block order", () => {
    ...
    expect(outputDefinitions.every((definition) => definition.variants.length >= 2)).toBe(true)
    expect(outputDefinitions.some((definition) => String(definition.key) === definition.astBlockType)).toBe(false)
    expect(outputDefinitions.filter((definition) => definition.astBlockType === "code").map((definition) => definition.key)).toEqual([
      "naver-se4:code",
      "naver-se3:code",
      "naver-se2:code",
    ])
