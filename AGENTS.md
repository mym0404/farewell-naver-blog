...
## Project Overview
- 이 저장소는 네이버 블로그 공개 글을 수집해 editor별 parser capability 기준으로 공용 AST로 변환하고 Markdown, frontmatter, 자산 파일, `manifest.json`으로 export하는 도구다.
...
- parser/sample seam은 `src/modules/parser/PostParser.ts`, `src/modules/parser/editors/*`, `src/modules/parser/blocks/*`, `src/shared/ParserCapabilities.ts`, `src/shared/SampleCorpus.ts`다.
...
## Operating Rules
- source of truth 우선순위는 사용자 지시, 루트 `AGENTS.md`, 코드/설정/테스트, evergreen knowledge, reference/generated 문서다.
- `.agents/knowledge/reference/`와 generated 보고서는 참고 자료다. 제품 계약이나 구조 계약의 source of truth가 아니다.
...
- parser capability, sample fixture, renderer/exporter 계약이 바뀌면 관련 knowledge와 generated 문서를 함께 갱신한다.
- generated 문서는 수동 편집하지 않고 `pnpm quality:report`로 갱신한다.
- commit, push, PR 생성은 사용자가 명시적으로 요청한 경우에만 수행한다.

## Validation Routes
- `pnpm check:local`: 저장소 파일을 수정한 모든 턴에서 항상 실행한다. `typecheck`, offline tests, `parser:check`, `samples:verify`를 실행한다.
- `pnpm check:full`: generated 품질 보고서, 기본 회귀, Playwright smoke UI까지 포함한 넓은 로컬 회귀다.
